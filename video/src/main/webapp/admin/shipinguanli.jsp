...
<link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/easyui/icon.css" />
 ...
 $(function(){
	...
	var userid='<%=session.getAttribute("userId")%>';
	 var opts = {
				method : 'post',
				url:'${pageContext.request.contextPath}/ction/selectShipinByname?name=',//提取所有
				nowrap : false,
				striped : true,
				fit : true,
				fitColumns : true,
			    ...
			    columns:[[    
			        ...
			        {field:'riqi',title:'上传日期',align: 'center',width:320}, 
	...
	$("#add").click(function(){
		...
		var url='';
		if(flage=="insert"){
			url='${pageContext.request.contextPath}/ction/insertShipin';
		}else{
			url='${pageContext.request.contextPath}/ction/updateShipin';
		}
    ...
    var $doc = $(document);
	 ...
	 $doc.delegate('[name=btnRemove]','click',function(event){
		...
		$.ajax({
			type:"post",
			url:"${pageContext.request.contextPath}/ction/deleteShipin",
			data:{"id":rowdata.id},
			error:function(){},
	    	timeout:1000,
	...
	$("#search").click(function(){
		...
		$.ajax({
			type:"post",
			url:"${pageContext.request.contextPath}/ction/selectShipinByname",
			data:{"name":name},
			error:function(){},
	    	timeout:1000,
			success:function(data){
...
<h2>视频信息</h2>
	 ...
	 <div id="ttt" style="padding:3px">
		 <div>
		  <a href="#" id="addFile"class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true">上传视频</a>
