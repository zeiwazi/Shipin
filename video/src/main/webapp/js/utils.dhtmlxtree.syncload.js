(function($){
	 ...
	 */
	utils.extDhtmlXTreeObjectEnabledSyncLoad = function(tree,func_url,func_object,func_afterOpen)
	{
		tree._correctPlus=function(itemObject)
		{
			...
			else if ((itemObject.childsCount)||(itemObject.unParsed)||(itemObject.enabledSyncLoad != undefined))
			{
				if ( /*((itemObject.htmlNode.childNodes[0].childNodes[1])&&(itemObject.htmlNode.childNodes[0].childNodes[1].style.display!="none"))
					||(!itemObject.enabledSyncLoad)*/
					this._isOpening(itemObject)||(!itemObject.enabledSyncLoad) )
				{
		...
		if(func_url)
		{
			tree.setOnOpenHandler(function(itemId){
				...
				if(item.unParsed && !item.childsCount){
					...
					utils.block(tree.parentObject,{themedCSS:{width:'0px',height:'0px'}});
					utils.stdPost(func_url.call(this,itemId),null,function(result){
						item.enabledSyncLoad = result.data.length;
						$.each(result.data,function(i,d){
							var o = func_object(d);
							var item = tree.addItem(o.pid,o.id,o.text,o.isfolder || o.isFolder,o.itemActionHandler,o.img1,o.img2,o.img3,o.optionStr);
							var key = o.key || '_defData';
							tree.setUserData(item.id,key,d);
						});
						if(!result.data.length)
							tree._correctPlus(item);
						if( func_afterOpen )
							func_afterOpen.call(tree,result,itemId);
						utils.unblock(tree.parentObject);
					},function(result){
	...
	utils.extDhtmlXTreeObjectSyncLoad = function(tree,itemId)
	{
